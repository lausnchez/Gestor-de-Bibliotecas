...
public class Biblioteca {    
    ...
    public Biblioteca(String nombre, String provincia, String telefono){
...
// Método para agregar una biblioteca a la base de datos
    ...
    /**
     * Método para obtener el id de una biblioteca por su nombre exacto
     * @param busqueda
     * @return 
     */
    public static Biblioteca obtenerIdBibliotecaPorNombreUnico(String busqueda) {
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        ...
        try {
            conn = BaseDatos.obtenerConnection();
            String sql = "SELECT id_biblio FROM bibliotecas WHERE nombre_biblio LIKE ?";
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, busqueda);
            rs = stmt.executeQuery();
            if(rs.next()) {
                ...
                String tlf = rs.getString("telefono_biblio");
                String email = rs.getString("email_biblio");
                resultado = new Biblioteca(id, nombre, provincia, ciudad, calle, tlf, email);
            }
            if(resultado == null){
                JOptionPane.showMessageDialog(null, "No se encontraron resultados", "Error", JOptionPane.INFORMATION_MESSAGE);
                return resultado;
    ...
    /**
     * Obtiene el nombre de una biblioteca pasándole su ID
     * @param id
     * @return 
     */
    public static String obtenerNombreBibliotecaPorID(int id){
        Biblioteca biblio = Biblioteca.obtenerBibliotecaPorId(id);
